...
const translations = {
  fr: {
    ...
    feelsLike: "Ressentie",
    humidity: "Humidité",
    humidityLevel: "Niveau d'humidité",
    wind: "Vent",
    direction: "Direction",
    pressure: "Pression",
    atmosphericPressure: "Pression atmosphérique",
    temperature24h: "Température sur 24h",
    windAndHumidity: "Vent et Humidité",
    ...
    locationDenied: "L'accès à la géolocalisation a été refusé.",
    locationUnavailable:
    ...
    locationNotSupported: "Géolocalisation non supportée par votre navigateur.",
  ...
  en: {
    ...
    temperature24h: "24h Temperature",
    windAndHumidity: "Wind and Humidity",
    ...
    locationErrorDefault: "Location denied. Using default city.",
    locationDenied: "Access to geolocation was denied.",
    locationUnavailable: "Location information is not available.",
    locationTimeout: "The location request timed out.",
    locationUnknownError: "An unknown error occurred during geolocation.",
...
const Admin = () => {
  ...
  const fetchWeatherData = async () => {
    try {
      ...
      if (userLocation) {
        currentUrl = `${OPENWEATHER_BASE_URL}/weather?lat=${userLocation.lat}&lon=${userLocation.lon}&appid=${OPENWEATHER_API_KEY}&units=metric&lang=${langParam}`;
        forecastUrl = `${OPENWEATHER_BASE_URL}/forecast?lat=${userLocation.lat}&lon=${userLocation.lon}&appid=${OPENWEATHER_API_KEY}&units=metric&lang=${langParam}&cnt=40`;
      } else {
        currentUrl = `${OPENWEATHER_BASE_URL}/weather?q=${city}&appid=${OPENWEATHER_API_KEY}&units=metric&lang=${langParam}`;
        forecastUrl = `${OPENWEATHER_BASE_URL}/forecast?q=${city}&appid=${OPENWEATHER_API_KEY}&units=metric&lang=${langParam}&cnt=40`;
      }
      ...
      if (currentData.cod !== 200)
        throw new Error(currentData.message || "Unknown error");
      ...
      >
        ...
        >
          <div className="flex flex-col h-full">
            ...
            {/* Messages d'erreur ou précision de localisation */}
            {locationError && (
              <div
            ...
            {locationAccuracy && (
              <div
                ...
                dir={dir}
              >
                {t("locationAccuracy")}: ±{Math.round(locationAccuracy)}{" "}
                {t("meters")}
              </div>
            )}
              ...
              <ul className="space-y-2">
                ...
                <li>
                  <a href="#alerts" className="text-blue-500 hover:underline">
                    Gérer les types et seuils d'alerte
                  </a>
                </li>
                <li>
                  <a href="#data" className="text-blue-500 hover:underline">
                    Accéder à toutes les données météo et alertes
                  </a>
                </li>
              </ul>
            </div>
            ...
            {userLocation && (
              ...
              >
                <h2 className="text-lg font-semibold mb-2 flex items-center gap-2">
                  <Navigation size={16} /> {t("locationMap")}
                </h2>
            ...
            >
              <h2 className="text-lg font-semibold mb-2 flex items-center gap-2">
                <Calendar size={16} /> {t("forecast7Days")}
              </h2>
        ...
        {/* Main Content */}
        <main className="flex-1 p-4 md:p-6 lg:p-8 overflow-y-auto" dir={dir}>
          <div
          ...
          {weatherData && (
            ...
            >
              <div
                className={`flex flex-col md:flex-row items-start md:items-center justify-between mb-4 md:mb-6 gap-3 ${
                  dir === "rtl" ? "md:flex-row-reverse" : ""
                }`}
              >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <DashboardCard
                  ...
                  description={`${t("feelsLike")}: ${Math.round(
                ...
                <DashboardCard
                  ...
                  description={`${t("direction")}: ${weatherData.wind.deg}°`}
                  darkMode={darkMode}
                  dir={dir}
                />
                <DashboardCard
